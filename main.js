...
//Sending patches into to DB
function savePatchesToDB(patches) {
  const query = `
    INSERT INTO system_patches (hotFixID, description, installedOn, installedBy)
    VALUES (?, ?, ?, ?)
  `;
  ...
  patches.forEach((patch) => {
    connection.query(query, [
      ...
      patch.installedOn || 'N/A',
      patch.installedBy || 'N/A',
    ], (err) => {
      if (err) {
        console.error('Error saving patch to database:', err.message);
      } else {
        console.log('Patch saved successfully:', patch.hotFixID);
      }
    });
  });
}

// Listen for system information from the renderer
ipcMain.on('save-system-info', (event, systemInfo) => {
  if (systemInfo && Array.isArray(systemInfo.patches)) {
    savePatchesToDB(systemInfo.patches);
    event.reply('save-system-info-response', { status: 'success' });
  } else {
    event.reply('save-system-info-response', { status: 'error', message: 'Invalid system info data.' });
  }
});
